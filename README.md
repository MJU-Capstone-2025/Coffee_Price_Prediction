...
학부 때 진행한 프로젝트를 바탕으로 데이터 수집, 시간 정렬, EDA, 모델 비교를 다시 구성했습니다. 현재는 Attention-LSTM·앙상블 비교와 로컬 서빙 경로를 구현했습니다. 5·20일 예측은 가격 유지 기준을 사용하고, 60일은 가격+거시 DLinear를 후보로 서빙합니다. 이 선택은 기존 노트북의 단일 구간 비교에 근거하므로 안정적인 우위를 확정한 것은 아닙니다.
